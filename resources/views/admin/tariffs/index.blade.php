                                ...
                                @foreach ($tariffs as $tariff)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                            {{ $tariff->min_usage }} - {{ $tariff->max_usage ?? '∞' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                            Rp {{ number_format($tariff->price_per_m3, 0, ',', '.') }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('admin.tariffs.edit', $tariff) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 mr-4">Edit</a>
                                            <form action="{{ route('admin.tariffs.destroy', $tariff) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this tariff?');">
                                                @csrf
                                                @method('DELETE')
